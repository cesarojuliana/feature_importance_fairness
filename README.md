...
We use four datasets for the study: Adult, German, Default and COMPAS. These datasets are often discussed with respect to fairness. The datasets used are available in folder `data`, and the results obtained are in folder `results`. In `utils.py` are implemented the functions used to calculate fairness measures, and to evaluate feature importance result.
